...
** Last update Fri Jan 13 17:13:54 2017 Gwendal Bazin
*/

#include "wolf3d.h"

void	sett(t_ray *ray, sfVector2f pos, float direction)
{
...
void	hitt(sfVector2i mapSize, t_ray *ray, sfVector2f *sidedist, int **map)
{
  while (!ray->hit)
    {
...
void	calc_dirt(t_ray *ray, sfVector2f *sidedist, sfVector2f pos)
{
  if (ray->raydirx < 0)
    {
      ray->stepx = -1;
...
float		raycastt(sfVector2f pos, float direction,
			int **map, sfVector2i mapSize)
{
  ...
  sett(&ray, pos, direction);
  calc_dirt(&ray, &sidedist, pos);
  hitt(mapSize, &ray, &sidedist, map);
  if (ray.side == 0)
